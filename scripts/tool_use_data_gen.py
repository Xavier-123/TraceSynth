...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/tool_use_data_gen.yaml',
    ...
    def run_tasks_from_file(config: Dict[str, Any]):
        ...
        if os.path.isdir(source_path):
            ...
            select_datasets = datasets.select(range(2))
            for dataset in select_datasets:
                if dataset['id'] in processed_ids:
                    logger.info(f"⏭️  Skipping processed task: {dataset['id']}")
                    continue

                task_data = {"persona": dataset["question"], "id": dataset["id"]}
            ...
            try:
                with open(source_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        ...
                        # Break if we've reached the max tasks limit
                        if config["processing"]["max_tasks"] and len(tasks_to_process) >= config["processing"][
                            "max_tasks"]:
                            break
