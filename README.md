...
`configs/persona_1.jsonl` 每行一条 JSON：

```json
{"persona": "在进行创业投资时，你如何确定一个项目的估值？", "id": "rag-2"}
```

也支持 HuggingFace `datasets` 目录作为 `paths.data_file`。
...
成功任务写入 `output/virtual_tool_use.jsonl`，每条记录：

```json
{"id": "rag-2", "fuzzy_task": "...", "checked_tools": [...]}
```
...
| `more_info.json` | 约束规则、背景、工作流、合成复杂度参数 |
...
- 提示词模板集中在 [`tracesynth/functions/prompt.py`](tracesynth/functions/prompt.py)，修改流程描述或输出格式时优先改此文件
