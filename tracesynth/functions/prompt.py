...
任务要求：结合给定背景信息，设计一套针对特定知识库场景的 Agentic RAG 执行方案。方案须覆盖全部 4 类 RAG 工具、对齐 step2~step5 四个必经步骤，共设计 {num_tools} RAG 工具。同时针对该场景设计自定义 RAG 组件——功能聚焦单一（如：领域实体链接、时效性过滤、表格片段结构化提取），不可直接生成最终答案或替代完整检索链路。
...
### 步骤3：RAG 工具清单设计
包含标准流水线工具及为该场景定制的虚拟组件。须设计 **{num_tools}** 工具，每款工具须标注所属阶段（检索前优化 / 检索 / 检索后优化 / 相关性评估）。格式严格遵循：
{{
    "name": "工具名称(英文命名)",
...
1. 若某工具的核心功能完全可由智能体自身完成，则不应单独拆分为独立工具。
   ...
   - 保留：**检索相关性评估器**——虽可用 LLM 完成，但作为流水线显式步骤的工具应保留，因其输出（缺口诊断、是否迭代）驱动步骤5到步骤2 的回路。
...
4. 最终校验：仅依靠修改后的工具集，智能体须能完整走通 Agentic RAG 六步流程（{iteration_requirement}）。
...
你是面向基于 LangGraph 构建的智能体检索增强生成（Agentic RAG）流水线的规划智能体。在正式执行前生成一套完整的工具调用执行轨迹（能完美解决用户提问）。仅返回一个合法 JSON 对象，内部包含 plan 字段。
'''
...
你是智能体检索增强生成（Agentic RAG）工具调用轨迹的评估员。仅当出现以下情形时判定方案无效：方案无法正常执行、违反合规规则、已有对应工具却缺失检索增强生成的必要流程阶段，或方案执行完仍无法形成对用户原始问题的直接回答。仅返回一份合法 JSON 对象。
'''
...
## 评估维度：
工具调用合法性、必填参数完备性、流程完整覆盖度、依赖关系正确性、迭代逻辑设计、规则合规性、端到端任务可解性。
端到端任务可解性：若按该方案完整执行，是否能够获得回答用户原始问题所需的信息、证据和最终组织答案的依据。
仅出现以下任一情况时，判定 is_valid=false（方案无效）：
步骤调用了不在可用工具列表中的工具，或是缺少必填入参
系统已有对应工具，但方案缺少查询优化、检索、后处理、效果评估任一核心环节
方案违反合规限制，或现有内容无法正常执行
方案虽然结构完整，但执行完后仍无法形成对用户原始问题的直接回答，例如缺少必要证据获取、没有覆盖问题中的关键实体 / 条件 / 时间范围，或流程目标与用户问题不一致

出现以下情况时判定 is_valid=true（方案有效），当方案有效时，其它字段可以为空：
depends_on（依赖项）、parameter_sources（参数来源）信息不完善，但所有步骤参数均可正常运行
迭代设计有优化空间，但整套方案在规则内能够正常执行
步骤顺序、说明文档存在轻微瑕疵
方案不保证工具一定返回正确答案，但只要流程能合理获取、筛选、评估并组织回答用户问题所需信息，即可判定有效
...
- 是否在信息不足时正确触发步骤5到步骤2 的迭代回路（而非单次检索即作答）
